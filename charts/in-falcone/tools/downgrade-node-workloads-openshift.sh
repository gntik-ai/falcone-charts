...
helm upgrade "$release" "$package" --version "$version" --namespace "$namespace" --wait --timeout 15m --reuse-values --values "$(dirname "$0")/../values/downgrade-node-workloads-openshift.yaml"
"$(dirname "$0")/verify-node-workloads.sh" --namespace "$namespace" --release "$release" --platform openshift
