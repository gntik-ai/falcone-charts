# In Falcone Helm deployment guide
    ...
    sources:
      apisixAdminKey:
        ...
        externalRef:
          ...
          reference: secret/data/in-falcone/platform/apisix-admin
```
...
helm package charts/in-falcone --destination dist/charts
```
...
helm upgrade --install in-falcone charts/in-falcone \
  ...
  -f charts/in-falcone/values/customer-reference.yaml \
...
```bash
helm upgrade --install in-falcone charts/in-falcone \
  --namespace in-falcone-prod \
  --create-namespace \
  -f charts/in-falcone/values.yaml \
  -f charts/in-falcone/values/profiles/standard.yaml \
  -f charts/in-falcone/values/prod.yaml \
  -f charts/in-falcone/values/platform-kubernetes-loadbalancer.yaml
```
...
```bash
helm dependency build charts/in-falcone
helm upgrade in-falcone charts/in-falcone \
  --namespace in-falcone \
  -f charts/in-falcone/values.yaml \
  -f charts/in-falcone/values/profiles/standard.yaml \
  -f charts/in-falcone/values/prod.yaml \
...
```bash
helm history in-falcone --namespace in-falcone
helm rollback in-falcone <REVISION> --namespace in-falcone
kubectl get configmap in-falcone-bootstrap-state --namespace in-falcone
kubectl get jobs --namespace in-falcone | grep bootstrap
```
