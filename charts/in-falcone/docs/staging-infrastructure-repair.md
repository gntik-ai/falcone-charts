# Staging infrastructure repair (chart 0.4.5)
...
Chart 0.4.5 carries the 0.4.3 repair and the same six first-party images that
chart 0.4.4 repinned to Falcone main `61540248`, without changing public APIs or
Falcone product source. Chart 0.4.5 corrects only the exact semantic-diff gate
boundary that blocked the 0.4.4 real dry-run — where the secret-suppressed owner
diff mis-classified an unrelated External Secrets object — and is an immutable
republish with no overwrite of the published 0.4.4 artifact and no API, schema,
...
- `Revision20BackupEvidence` identifies exact context/namespace/release,
  revision 20, chart 0.4.1, target repair chart 0.4.5, published package digest,
...
For a real apply the tool pulls chart 0.4.5 from
...
charts/in-falcone/migrations/revision-20-repair.sh \
  ...
  --confirm-target 'default/in-falcone-staging/falcone@20/in-falcone-0.4.1->in-falcone-0.4.5/sha256:PUBLISHED_PACKAGE_DIGEST'
```
...
It binds the original 20/0.4.1 source, the actual current revision, chart 0.4.5,
...
charts/in-falcone/migrations/revision-20-repair.sh \
  ...
  --confirm-target 'default/in-falcone-staging/falcone@CURRENT_REVISION/in-falcone-0.4.5/sha256:PUBLISHED_PACKAGE_DIGEST' \
...
`default/in-falcone-staging/falcone@CURRENT_REVISION/in-falcone-0.4.5/sha256:PUBLISHED_PACKAGE_DIGEST`
...
The supported repair anchor is chart 0.4.1 revision 20 to chart 0.4.5,
