# Migration: Revision 20 to chart 0.4.5

## Preconditions and evidence

- Exact context `default`, namespace `in-falcone-staging`, release `falcone`,
  revision `20`, and starting chart `in-falcone-0.4.1`.
- A published chart 0.4.5 package digest, not a source-only estimate. Chart
  0.4.4 remains the historical published artifact whose real dry-run was
  blocked by the ESO classification gate; it is not overwritten or used as
  the active repair target.
...
- Apply pulls the published 0.4.5 OCI artifact, verifies the registry-reported
...
20, source chart 0.4.1, target chart 0.4.5 and package digest. It retains the
...
metadata, and reapplies chart 0.4.5. It never deletes storage. Once data exists,
