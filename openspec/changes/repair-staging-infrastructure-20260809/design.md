# Design: Revision-20/r24 staging infrastructure repair to 0.4.16
...
application policies in its canary lookup. The 0.4.16 correction must use the
...
- Make exact r24 recovery converge OpenBao Kubernetes auth from the verified
  0.4.16 package before exposing the new ESO identity through the store.
...
8. Apply requires separate fresh backup and parity attestations bound to exact
   ...
   apply pulls the 0.4.16 OCI artifact, verifies its registry-reported digest, and
...
13. Revision 23 is admitted only as the exact failed chart-0.4.9 canceled upgrade
    ...
    existing two-pass Phase-A path and requires fresh 0.4.16/package-bound backup
...
16. Exact r24 recovery inserts an auth-first gate before the existing store,
    ...
    auth-reconcile Job from the digest-verified 0.4.16 package with
    ...
    `in-falcone.io/recovery-target-chart=in-falcone-0.4.16`, and
...
19. Chart 0.4.12 remains an immutable failed-recovery artifact; chart 0.4.13
    ...
    its ExternalSecret precursor gate was incomplete. The correction is a new
    immutable chart 0.4.16; evidence, confirmations, release material, recovery
    paths, and operator procedures all bind 0.4.16 rather than rewriting
    historical packages.
...
22. The additional r24 precursor is one exact 0.4.14 partial state: desired store
    ...
    that desired store; it creates a fresh 0.4.16 digest-bound Job before
...
23. The early Phase-A confirmation gate binds the exact current revision/chart
    to immutable target 0.4.16 and a digest-shaped suffix before attestations are
    loaded. A 0.4.15 confirmation therefore fails with
...
0.4.11, 0.4.12, 0.4.13, 0.4.14, or 0.4.15 is a rollback target. After PVC
...
1. Publish immutable 0.4.16 and bind fresh backup/parity evidence plus the exact
   r24→0.4.16 confirmation to its registry digest.
...
6. Keep Phase B separately JIT-gated. Any failure after mutation is recovered by
   forward-applying 0.4.16; no Helm rollback is allowed.
...
and 0.4.16 package checksum validation run on the source commit. Operator and
