/**
 * Public black-box contracts for the 0.4.16 OpenBao self-token policy repair.
...
test('platform policy grants only exact lookup-self and revoke-self capabilities', () => {
  ...
  assert.equal(publicChartVersion(), '0.4.16', 'the corrected public package must be 0.4.16')
})
...
test('r24 fail-forward admits the exact published 0.4.14 lookup-self 403 precursor', () => {
  ...
  assertFresh016Job(creations[0], 'exact self-token precursor')
  ...
  assert.equal(version, '0.4.16', 'the recovery target must be public chart 0.4.16')
})
...
test('r24 retry retains the 0.4.14 failed Job and creates a new 0.4.16 identity per attempt', () => {
  const version = publicChartVersion()
  const result = runRevision24SelfTokenPolicyRecovery({
    targetVersion: version,
    attemptCount: 2,
  ...
  creations.forEach((creation) => assertFresh016Job(creation, 'self-token retry'))
  ...
  assert.equal(version, '0.4.16', 'the retry target must be public chart 0.4.16')
})
