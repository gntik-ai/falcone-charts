/**
 * Public black-box contracts for the 0.4.19 OpenBao self-token policy repair.
...
test('r24 fail-forward admits the exact published 0.4.14 lookup-self 403 precursor', () => {
  ...
  assertFresh019Job(creations[0], 'exact self-token precursor')
  ...
  assert.equal(version, '0.4.19', 'the recovery target must be public chart 0.4.19')
})
...
test('r24 retry retains the 0.4.14, 0.4.16, and 0.4.17 failed Jobs and creates a new 0.4.19 identity per attempt', () => {
  const version = publicChartVersion()
  const result = runRevision24SelfTokenPolicyRecovery({
    targetVersion: version,
    attemptCount: 2,
  ...
  creations.forEach((creation) => assertFresh019Job(creation, 'self-token retry'))
  ...
  assert.equal(version, '0.4.19', 'the retry target must be public chart 0.4.19')
})
