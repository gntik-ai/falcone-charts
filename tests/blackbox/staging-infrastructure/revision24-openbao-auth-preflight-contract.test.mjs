...
function assertMaterializedAuthJob(creation, context) {
  ...
  assert.equal(creation.chart, 'in-falcone-0.4.17')
...
function assertRecoveryRootOnlyForAuthJob(lines, executionIndex, context) {
  ...
  assert.match(rootCall, /(?:^|\s)--version 0\.4\.17(?:\s|$)/)
...
test('exact r24 recovery runs the attested 0.4.17 auth Job before CAS handoff and preserves both health gates', () => {
  const result = runRevision24AuthRecovery()
  assert.equal(
    result.status,
    0,
    `exact 0.4.17 auth-first recovery failed\nstdout:\n${result.stdout}\nstderr:\n${result.stderr}\ntrace:\n${result.trace}`,
  )
  assert.match(result.stdout, /revision24-global-wait-recovery=validated/)
  assert.match(result.stdout, /chart=in-falcone-0\.4\.17/)
  ...
  for (const upgradeIndex of upgrades) {
    assert.match(lines[upgradeIndex], /(?:^|\s)--version 0\.4\.17(?:\s|$)/)
