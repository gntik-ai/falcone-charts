...
const phaseAConfirmation = `default/in-falcone-staging/falcone@20/in-falcone-0.4.1->in-falcone-0.4.7/${repairDigest}`
const phaseBConfirmation = `default/in-falcone-staging/falcone@22/in-falcone-0.4.7/${repairDigest}`
...
test('repair and forward-recovery pin chart 0.4.7 and reject 0.4.6, 0.4.5, 0.4.4, and 0.4.3 target confirmations', () => {
  for (const [label, tool, args, scenario] of [
    ...
    const invocation = invokeMigration(tool, args, scenario)
    try {
      assertSuccess(invocation.result, `${label} with the immutable 0.4.7 repair package`)
      ...
      for (const call of chartCalls) {
        assert.match(call, /(?:^|\s)--version 0\.4\.7(?:\s|$)/, `${label} did not select chart 0.4.7`)
        assert.doesNotMatch(call, /(?:^|\s)--version 0\.4\.[3456](?:\s|$)/,
  ...
  for (const historicalVersion of ['0.4.6', '0.4.5', '0.4.4', '0.4.3']) {
    const obsoleteConfirmation = phaseBConfirmation.replace('in-falcone-0.4.7', `in-falcone-${historicalVersion}`)
    for (const [label, tool, args, scenario] of [
      ...
      try {
        ...
        assert.match(combined(invocation.result), /in-falcone-0\.4\.7/)
...
test('repair and forward use the packaged chart top-level version and reject a wrong top-level version before mutation', () => {
  ...
  for (const [label, tool, args, baseScenario] of [
    ...
    try {
      ...
      if (!valid.helmCalls.includes('fixture-package first-version=0.2.2 top-level-version=0.4.7')) {
